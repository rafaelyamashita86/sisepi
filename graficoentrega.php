    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ...
                <?php
                include 'conecta.php';
                $sql = "SELECT epi.descricao,entrega.quantidade FROM entrega,epi WHERE entrega.id_epi=epi.id GROUP BY id_epi;";
                $consulta = mysqli_query($conn, $sql);
                while ($dados = mysqli_fetch_array($consulta)) {
                    $quantidade = $dados['quantidade'];
                    $id_epi = $dados['descricao'];

                ?>['<?php echo $id_epi ?>', <?php echo $quantidade ?>],
                <?php } ?>
            ]);
            var options = {
                title: 'Entregas Realizadas por EPI',
                is3D: true,
            };
